...
#         return {"card_number": card_number , "expiration_date": expiration_date, "ccv": ccv}

#     def mask_data(card_number, expiration_date, ccv):

def process_payment_cc(total) -> None:
    card_number = input("Please enter your credit card number: ")
    expiration_date = input("Please enter your credit card expiration date: ")
    ccv = input("Please enter your credit card CCV: ")
...
def process_payment_paypal(total) -> None:
...
def handle_payment(total: Decimal) -> None:
    # Ask for the type of payment, call the mapping of the method
    # THis method knows how to handle the payments
    payment_type = input(
...
class ShoppingCart:
    ...
    # The following method has the problem that if we want to add a new method, then we need to add a new method here 
    # All of them have in common: input some values and the printing of processing the payment (with all of them masking the values so no secrets are shown) 
    ...
    #         password_masked = len(password) * "*"
    #         print(
    #             f"Processing PayPal payment of ${self.total:.2f} with username {username} and password {password_masked}..."
    #         )
    #     elif payment_type == "apple":
    #         device_id = input("Please enter your Apple Pay device ID: ")
    #         device_id_masked = device_id[-4:].rjust(len(device_id), "*")
    #         print(
    #             f"Processing Apple Pay payment of ${self.total:.2f} with device ID {device_id_masked}..."
    #         )
    #            else:
